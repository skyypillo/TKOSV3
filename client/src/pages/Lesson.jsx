...
function Lesson() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const response = await fetch(
        `http://localhost:5050/lesson/${params.id.toString()}`
      );
      if (!response.ok) {
  ...
  async function onSubmit(e) {
    ...
    const updatedComments = [...lesson.comments, updatedComment];
    const user = { ...lesson, comments: updatedComments };

    try {
      let response;
      response = await fetch(`http://localhost:5050/lesson/${params.id}`, {
        method: "PATCH",
  ...
  return (
    <div className=" w-full py-16 px-20">
      ...
      <div className="w-full px-20 mt-4">
        <audio ref={audioRef} src={`/AUDIO/${lesson._id}.mp3`} />
      </div>
